...
const WIKI_NAME = 'List of Disney+ original programming';

async function runAll() {
  const result = await runStreamNetwork(WIKI_NAME);

  await fs.promises.writeFile(require('path').join(__dirname, `dist/disney+_all.json`), JSON.stringify(result, undefined, 2), { encoding: 'utf-8' });
}

async function runAiredTV() {
  const result = filterUpcoming(filterUnscripted(filterFilm(await runStreamNetwork(WIKI_NAME))));

  await fs.promises.writeFile(require('path').join(__dirname, `dist/disney+_tv_aired.json`), JSON.stringify(result, undefined, 2), { encoding: 'utf-8' });
}
